...
class CrawlerClient:
    ...
    def __init__(self, login_user="", login_password="", *args, **kwargs):
    ...
    def request_login(self):
        ...
        if response.status_code != 200:
            raise Exception(
                f"The website login has failed, server responded \
    ...
    def request_benefits(self, cpf):
        ...
        if response.status_code != 200:
            raise Exception(
                f"The website benefits endpoint has failed, server \
    ...
    def get_benefits(self, cpf, complete=False):
        ...
        if not self.auth_token:
            raise Exception(
                "The website login has failed, the authorization \
                 token was not saved"
            )
